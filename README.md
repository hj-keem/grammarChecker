...
- [ 마이페이지 ]   
  📍 마이페이지 (최초로그인 시 프로필 자동 생성)      
  로그인 성공 시 현재 로그인된 사용자의 프로필이 존재하는지 확인. 기존에 프로필이 없다면 사용자 프로필 생성        
  카카오 로그인 시 사용자의 이름을 받아와 username에 고정값으로 지정,    
  이외에 nickname이나 사용자 프로필 사진은 아래 '변경하기' 버튼을 이용해 변경 가능
